...
def generate_samples(vae, cfg):
    sample = torch.randn(cfg.SOLVER.BATCH_SIZE).to(device)
    res = vae.dec_forward(sample)
    return res


def train(cfg, model, train_loader, tid, mem, logger, writer, metrics, num_iter=1):
    vae, classifier = model
    vae.train()
    classifier.model()
    ...
    for epoch_idx in range(0, cfg.SOLVER.NUM_EPOCHS):
        for batch_idx, data in enumerate(train_loader):
            for _ in range(num_iter):
                ...
                sampled_mem, _ = generate_samples(vae, cfg)
